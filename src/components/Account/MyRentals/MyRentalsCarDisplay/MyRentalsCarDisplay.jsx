...
const MyRentalsCarDisplay = ({ car, providerId }) => {
  ...
  const handleSave = async () => {
    try {
      const updatedCar = await makeApiRequest(`https://norwegian-rental.online/api/cars/${car.id}`, {
        method: 'PUT',
...
const handleExtraFeaturesUpdate = async (selectedFeatureIds) => {
 ...
 const updatedFeatures = await Promise.all(
  ...
  const fetchFeatureName = async (featureId) => {
    try {
      const featureData = await makeApiRequest(`https://norwegian-rental.online/api/extra-features/${featureId}`);
